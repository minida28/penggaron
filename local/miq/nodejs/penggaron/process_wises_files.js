...
async function run(loggerFile, savePath) {
	...
	processFile(loggerFile, savePath)
		.then(await processData(dataStr))
		.catch(function (error) {
			console.log(error.message);
			return;
		})
		.then(await connectMysql(connection))
		// .catch(function (error) {
		// 	console.log(error.message);
		// 	return;
		// })
		.then(async function (response) {
			...
			for (let index = 0; index < array.length; index++) {
				...
				var tableExist = await checkTable(connection, element);
				if (tableExist) {
					console.log('table name:', element, 'exist', 'number of lines:', dataObj[element].length);
					await insertData(connection, element, dataObj);
				}
				else
...
async function checkTable(connection, key) {
...
async function insertData(connection, key, objectvar) {

	console.log('Inserting to table', key, '...');

	var inserts = [];

	var sql = "REPLACE INTO " + key + " VALUES ";
	...
	// console.log('inserts_joined:', inserts.join(","));
